...
int main( int argc, char* argv[] )
{
	...
	Ptr<ERFilter> er_filter1 = createERFilterNM1( loadClassifierNM1( "trained_classifierNM1.xml" ), 8, 0.00015f, 0.93f, 0.2f, true, 0.1f );
	...
	Ptr<OCRTesseract> ocr = OCRTesseract::create( "D:\\deep learning/", "eng" );
	...
	float scale_font = 0.5;// ( float )( 2 - scale_img ) / 1.4f;
	...
	for( int i = 0; i<( int )nm_boxes.size(); i++ )
	{
		...
		rectangle( out_img_detection, nm_boxes[ i ].tl(), nm_boxes[ i ].br(), Scalar( 0, 255, 255 ), 1 );
		...
		{
			boxes[ j ].x += nm_boxes[ i ].x - 20;
			boxes[ j ].y += nm_boxes[ i ].y - 20;
			...
			rectangle( out_img, boxes[ j ].tl(), boxes[ j ].br() + Point( 5, 5 ), Scalar( 255, 0, 0 ), 1 );
			Size word_size = getTextSize( words[ j ], FONT_HERSHEY_SIMPLEX, ( double )scale_font, 1/*( int )( 3 * scale_font )*/, NULL );
			rectangle( out_img, boxes[ j ].tl() - Point( 3, word_size.height + 3 ), boxes[ j ].tl() + Point( word_size.width, 0 ), Scalar( 0, 0, 255 ), -1 );
			putText( out_img, words[ j ], boxes[ j ].tl() - Point( 1, 1 ), FONT_HERSHEY_SIMPLEX, scale_font, Scalar( 255, 255, 255 ), 1/*( int )( 3 * scale_font )*/ );
